...
class TestSceneSerialization extends TestCase
{
	...
	private function emptySceneAssertions(scene : MashScene) : Void
	{
		...
		assertEquals(0, Lambda.count(scene.ecs.entities));
	}
	...
	function initializeSimpleScene():Void 
	{
		for (i in 0...10) //1 through 10
		{
			...
			simpleScene.ecs.addEntity(entity);
			var a : TestComponentA = new TestComponentA((i-1) * 10);
			entity.add(a);
		}
		
		simpleScene.ecs.getEntityByName("Entity 3")
			.add(new TestComponentB("Some String On Entity 3"));
		
		simpleScene.ecs.getEntityByName("Entity 7")
	...
	private function simpleSceneAssertions(scene : MashScene) : Void
	{
		...
		assertEquals(10, Lambda.count(scene.ecs.entities));
		
		assertTrue(scene.ecs.getEntityByName("Entity 2").has(TestComponentA));
		assertTrue(scene.ecs.getEntityByName("Entity 7").has(TestComponentB));
		assertFalse(scene.ecs.getEntityByName("Entity 6").has(TestComponentB));
		assertEquals(5 * 10, scene.ecs.getEntityByName("Entity 6").get(TestComponentA).fieldA);
		assertEquals("Some String On Entity 3", scene.ecs.getEntityByName("Entity 3").get(TestComponentB).fieldA);
	}
	...
	function initializeComplexScene():Void 
	{
		...
		{
			var entity = new Entity("Entity " + i);
			complexScene.ecs.addEntity(entity);
			var a : TestComponentA = new TestComponentA((i-1) * 10);
			entity.add(a);
		}
		
		var entity1 = complexScene.ecs.getEntityByName("Entity 1");
		var entity2 = complexScene.ecs.getEntityByName("Entity 2");
		var entity3 = complexScene.ecs.getEntityByName("Entity 3");
		var entity4 = complexScene.ecs.getEntityByName("Entity 4");
	...
	private function complexSceneAssertions(scene : MashScene) : Void
	{
		...
		var entity1 = scene.ecs.getEntityByName("Entity 1");
		var entity2 = scene.ecs.getEntityByName("Entity 2");
		var entity3 = scene.ecs.getEntityByName("Entity 3");
		var entity4 = scene.ecs.getEntityByName("Entity 4");
